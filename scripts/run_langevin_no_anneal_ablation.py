...
CAMPAIGN_SLUG = "langevin_no_anneal_ablation"
CONFIG_HASH_VERSION = "langevin-no-anneal-ablation-effective-v1"
...
def run_id_for(seed: int, variant_slug: str) -> str:
    return f"seed{seed}_10k_{variant_slug}_{TARGET.lower()}"
    ...
    for seed in args.seeds:
        for variant in VARIANTS:
            ...
            entry = {
                ...
                "config_hash_basis": (
                    "resolved main config plus seed, Langevin no-anneal override, "
                    "10k epoch override, variant overrides, and user extra overrides; "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the 18-run Langevin no-anneal ablation sweep with dynamic GPU scheduling."
    )
