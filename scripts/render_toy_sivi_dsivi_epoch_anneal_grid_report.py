...
from run_toy_sivi_dsivi_epoch_anneal_grid import CAMPAIGN_SLUG  # noqa: E402


REPO_ROOT = SCRIPT_DIR.parent
CAMPAIGN_DIR = REPO_ROOT / "campaigns" / CAMPAIGN_SLUG
REPORT_NAME = "toy_sivi_dsivi_epoch_anneal_grid_report.md"
RUN_CSV_NAME = "toy_sivi_dsivi_epoch_anneal_grid_runs.csv"
SUMMARY_CSV_NAME = "toy_sivi_dsivi_epoch_anneal_grid_summary.csv"
        ...
        rows.append(
            {
                ...
                "seed": entry["seed"],
                "epochs": epochs,
                "annealing_steps": entry["annealing_steps"],
...
def build_summary_rows(run_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in run_rows:
        grouped[(str(row["regime"]), str(row["target"]), str(row["method"]))].append(row)

    summary: list[dict[str, Any]] = []
    for (regime, target, method), rows in sorted(grouped.items()):
        out: dict[str, Any] = {
            "regime": regime,
            "regime_label": rows[0].get("regime_label", regime),
            "target": target,
            "method": method,
            "method_slug": rows[0].get("method_slug", ""),
            "runs": len(rows),
            "epochs": rows[0].get("epochs", ""),
            "annealing_steps": rows[0].get("annealing_steps", ""),
            "reverse_sample_num": rows[0].get("reverse_sample_num", ""),
...
def render_markdown(summary_rows: list[dict[str, Any]], run_rows: list[dict[str, Any]]) -> str:
    lines = [
        "# Toy SIVI/DSIVI Epoch-Anneal Grid",
        "",
    ...
    regime_order = ["ep2000_a1000", "ep4000_a2000"]
    target_order = ["banana", "multimodal", "x_shaped", "8_gaussians"]
    method_order = {"DSIVI": 0, "SIVI-rsn4096": 1, "SIVI-rsn1024": 2}
    by_regime_target: dict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
    for row in summary_rows:
        by_regime_target[(str(row["regime"]), str(row["target"]))].append(row)

    label_by_regime = {
        str(row["regime"]): str(row.get("regime_label") or row["regime"])
        for row in summary_rows
    }
    for regime in regime_order:
        lines.extend([f"## {label_by_regime.get(regime, regime)}", ""])
        for target in target_order:
            rows = sorted(
                by_regime_target.get((regime, target), []),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render the toy SIVI/DSIVI epoch-anneal grid report.")
