...
from run_toy_sivi_dsivi_ema_grid import CAMPAIGN_SLUG  # noqa: E402


REPO_ROOT = SCRIPT_DIR.parent
CAMPAIGN_DIR = REPO_ROOT / "campaigns" / CAMPAIGN_SLUG
REPORT_NAME = "toy_sivi_dsivi_ema_grid_report.md"
RUN_CSV_NAME = "toy_sivi_dsivi_ema_grid_runs.csv"
SUMMARY_CSV_NAME = "toy_sivi_dsivi_ema_grid_summary.csv"
...
def build_summary_rows(run_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in run_rows:
        grouped[(str(row["target"]), str(row["method"]), float(row["ema_beta"]))].append(row)

    summary: list[dict[str, Any]] = []
    for (target, method, ema_beta), rows in sorted(grouped.items()):
        out: dict[str, Any] = {
            "target": target,
            "method": method,
...
def render_markdown(summary_rows: list[dict[str, Any]], run_rows: list[dict[str, Any]]) -> str:
    lines = [
        "# Toy SIVI/DSIVI EMA Grid",
        "",
        f"Completed runs summarized: {len(run_rows)}",
        "",
        "Default toy configs were used with EMA enabled. Metrics are final scalar values "
        "averaged over seeds. Training time uses `summary/total_training_time` when "
        "available; wall-clock time is the scheduler process duration.",
        "",
    ]
    target_order = ["banana", "multimodal", "x_shaped"]
    method_order = {"DSIVI": 0, "SIVI-rsn4096": 1}
    ...
    for target in target_order:
        rows = sorted(
            ...
            key=lambda item: (method_order.get(str(item["method"]), 99), float(item["ema_beta"])),
        )
        if not rows:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render the toy SIVI/DSIVI EMA grid report.")
