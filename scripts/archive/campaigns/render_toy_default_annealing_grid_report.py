...
from run_toy_default_annealing_grid import CAMPAIGN_SLUG  # noqa: E402


REPO_ROOT = SCRIPT_DIR.parents[2]
CAMPAIGN_DIR = REPO_ROOT / "campaigns" / CAMPAIGN_SLUG
REPORT_NAME = "toy_default_annealing_grid_report.md"
RUN_CSV_NAME = "toy_default_annealing_grid_runs.csv"
SUMMARY_CSV_NAME = "toy_default_annealing_grid_summary.csv"
...
def _best_value(points: list[tuple[int, float]], mode: str) -> float | None:
    if not points:
        return None
    return (max if mode == "max" else min)(points, key=lambda item: item[1])[1]
...
def build_run_rows(manifest: list[dict[str, Any]], completed: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for entry in manifest:
        ...
        rows.append(
            {
                "run_id": entry["run_id"],
                "annealing_mode": entry.get("annealing_mode", ""),
                "annealing_label": entry.get("annealing_label", ""),
                "annealing_enabled": entry.get("annealing_enabled", ""),
                "target": entry["target"],
                "method": entry["method"],
                ...
                "elbo_best": _best_value(metrics.get(ELBO_TAG, []), "max"),
                "w2_final": _last_value(metrics.get(W2_TAG, [])),
                "w2_best": _best_value(metrics.get(W2_TAG, []), "min"),
...
def build_summary_rows(run_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in run_rows:
        grouped[(str(row["annealing_mode"]), str(row["target"]), str(row["method"]))].append(row)

    summary: list[dict[str, Any]] = []
    for (annealing_mode, target, method), rows in sorted(grouped.items()):
        out: dict[str, Any] = {
            "annealing_mode": annealing_mode,
            "annealing_label": rows[0].get("annealing_label", annealing_mode),
            "annealing_enabled": rows[0].get("annealing_enabled", ""),
            "target": target,
            "method": method,
            "runs": len(rows),
        ...
        for metric in ("elbo_final", "elbo_best", "w2_final", "w2_best"):
            values = [float(row[metric]) for row in rows if row.get(metric) is not None]
            out[f"{metric}_mean"] = _mean(values)
            out[f"{metric}_sd"] = _stdev(values)
        summary.append(out)
    return summary
...
def render_markdown(summary_rows: list[dict[str, Any]], run_rows: list[dict[str, Any]]) -> str:
    lines = [
        "# Toy Default Annealing Grid",
        "",
        f"Completed runs summarized: {len(run_rows)}",
        "",
        "Metrics are final scalar values averaged over seeds. Training time uses the logged "
        "`summary/total_training_time` scalar when available; wall-clock time is the scheduler "
        "process duration.",
        "",
    ]
    mode_order = ["anneal_on", "anneal_off"]
    target_order = ["banana", "multimodal", "x_shaped", "8_gaussians"]
    method_order = {"SIVI": 0, "UIVI": 1, "AISIVI": 2, "DSIVI": 3}
    by_mode_target: dict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
    for row in summary_rows:
        by_mode_target[(str(row["annealing_mode"]), str(row["target"]))].append(row)

    label_by_mode = {
        str(row["annealing_mode"]): str(row.get("annealing_label") or row["annealing_mode"])
        for row in summary_rows
    }
    for mode in mode_order:
        lines.extend([f"## {label_by_mode.get(mode, mode)}", ""])
        for target in target_order:
            rows = sorted(
                by_mode_target.get((mode, target), []),
                key=lambda item: method_order.get(str(item["method"]), 99),
            )
            if not rows:
                continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render the toy default annealing grid report.")
