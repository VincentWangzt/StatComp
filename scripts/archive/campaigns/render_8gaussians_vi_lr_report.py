...
from run_8gaussians_vi_lr_grid import CAMPAIGN_SLUG  # noqa: E402


REPO_ROOT = SCRIPT_DIR.parents[2]
CAMPAIGN_DIR = REPO_ROOT / "campaigns" / CAMPAIGN_SLUG
OUT_DIR = CAMPAIGN_DIR / "generated_reports"
REPORT_MD = OUT_DIR / "8gaussians_vi_lr_report.md"
RUN_CSV = OUT_DIR / "8gaussians_vi_lr_runs.csv"
SUMMARY_CSV = OUT_DIR / "8gaussians_vi_lr_group_summary.csv"
...
def _best(points: list[tuple[int, float]], mode: str) -> tuple[int | None, float | None]:
    if not points:
        return None, None
    step, value = max(points, key=lambda item: item[1]) if mode == "max" else min(points, key=lambda item: item[1])
    return step, value
...
def build_run_rows(manifest: list[dict[str, Any]], completed: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for entry in manifest:
        ...
        row: dict[str, Any] = {
            ...
            "vi_lr": entry["vi_lr"],
            "vi_lr_label": entry["vi_lr_label"],
            "vi_lr_slug": entry["vi_lr_slug"],
            "epochs": epochs,
...
def build_group_rows(run_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in run_rows:
        grouped[(str(row["method"]), float(row["vi_lr"]))].append(row)

    rows: list[dict[str, Any]] = []
    for (method, vi_lr), method_rows in sorted(grouped.items()):
        out: dict[str, Any] = {
            "method": method,
            "vi_lr": vi_lr,
            "vi_lr_label": method_rows[0]["vi_lr_label"],
...
def render_markdown(
    ...
    lines.extend(
        _table(
            ["Run", "Method", "VI LR", "Seed", "ELBO final", "KL final", "W2 final", "Minutes"],
            [
                [
                    row["run_id"],
                    row["method"],
                ...
                for row in sorted(
                    run_rows,
                    key=lambda item: (str(item["method"]), float(item["vi_lr"]), int(item["seed"])),
                )
            ],
        )
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render the 8-Gaussians VI lr grid report.")
