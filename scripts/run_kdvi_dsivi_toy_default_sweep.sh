...
# final/best VI metrics across five seeds for each method/target pair.

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
CAMPAIGN_SLUG="kdvi_dsivi_toy_default_sweep"
...
bad_groups = {
    ...
    if sorted(seeds) != [0, 1, 7, 42, 43]
}
if bad_groups:
    errors.append(f"groups without exactly seeds 0,1,7,42,43: {len(bad_groups)}")
...
if method_counts != {"KDVI": 35, "DSIVI": 35}:
