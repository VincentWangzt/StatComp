...
# KDVI toy-target batch-size / MCMC-steps sweep.
#
# Uses the current per-target KDVI base configs and only overrides:
#   - train.batch_size in {128, 1024}
#   - train.kdvi.mcmc_steps in {1, 2, 5, 10}
#   - seed in {0, 1, 7}
#
...
# across three seeds for each target/batch-size/K recipe.

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
CAMPAIGN_SLUG="kdvi_toy_batch_k_sweep"
...
usage() {
    cat <<'EOF'
Usage: bash scripts/run_kdvi_toy_batch_k_sweep.sh [options]

Options:
  --dry-run           Generate and validate the 144-run manifest only.
...
generate_manifest() {
    ...
    local target seed batch_size mcmc_steps config_path metric_family recipe run_id

    printf 'run_id\trecipe_id\ttarget\tseed\tconfig_path\tbatch_size\tmcmc_steps\tmetric_family\n' >"${tmp_path}"

    for target in banana x_shaped multimodal 8_gaussians 8_gaussians_small Langevin_post; do
        ...
        for batch_size in 128 1024; do
            for mcmc_steps in 1 2 5 10; do
                recipe="KDVI-${target}-bs${batch_size}-k${mcmc_steps}"
                for seed in 0 1 7; do
                    run_id="${recipe}-seed${seed}"
                    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
                        "${run_id}" "${recipe}" "${target}" "${seed}" \
                        "${config_path}" "${batch_size}" "${mcmc_steps}" \
                        "${metric_family}" >>"${tmp_path}"
                done
            done
        done
    done
...
validate_manifest() {
    python - "${REPO_ROOT}" "${MANIFEST_PATH}" "${EXPECTED_RUNS}" <<'PY'
import csv
import sys
...
expected_batches = {"128", "1024"}
expected_steps = {"1", "2", "5", "10"}
...
for row in rows:
    ...
    batches.add(row["batch_size"])
    steps.add(row["mcmc_steps"])
...
if len(set(run_ids)) != expected_runs:
    errors.append("run IDs are not unique")
if len(groups) != expected_runs // len(expected_seeds):
    errors.append(
        f"expected {expected_runs // len(expected_seeds)} recipe groups, "
        f"found {len(groups)}"
    )
bad_groups = {
...
if "student_uc" in targets:
    errors.append("student_uc must be excluded from this sweep")
if batches != expected_batches:
    errors.append(f"unexpected batch sizes: {sorted(batches)}")
if steps != expected_steps:
    errors.append(f"unexpected mcmc_steps values: {sorted(steps)}")
metric_families = {(row["target"], row["metric_family"]) for row in rows}
if ("Langevin_post", "elm_w2") not in metric_families:
    errors.append("Langevin_post rows must use metric_family=elm_w2")
for target in expected_targets - {"Langevin_post"}:
    if (target, "kl_w2") not in metric_families:
...
print(
    "Manifest validated: "
    f"{len(rows)} runs, {len(groups)} target/batch/K groups, "
...
build_command() {
    ...
    output_ref=(
        ...
        "train.kdvi.mcmc_steps=${mcmc_steps}"
...
if ((DRY_RUN)); then
    ...
    IFS=$'\t' read -r run_id recipe target seed config_path batch_size mcmc_steps metric_family <<<"${first_job}"
    cmd=()
    build_command cmd 0 "${run_id}" "${recipe}" "${target}" "${seed}" \
        "${config_path}" "${batch_size}" "${mcmc_steps}" "${metric_family}"
    printf '  %q' "${cmd[@]}"
    printf '\n'
    exit 0
fi
...
def sort_key(row):
    return (
        ...
        BATCH_ORDER.index(int(row["batch_size"])),
        K_ORDER.index(int(row["mcmc_steps"])),
    )

rows = []
for recipe in sorted(recipe_meta, key=lambda key: sort_key(recipe_meta[key])):
    ...
    row = {
        ...
        "mcmc_steps": int(meta["mcmc_steps"]),
...
lines = [
    ...
    "",
    f"Complete target/batch/K groups: **{len(complete_rows)} / {len(rows)}**.",
...
for target in TARGET_ORDER:
    ...
    if not complete_target_rows:
        lines.extend(["No complete recipe groups yet.", ""])
        ...
        for row in pareto_rows:
            lines.append(
                f"| `{row['batch_size']}` | `{row['mcmc_steps']}` | "
    ...
    if target == "Langevin_post":
        ...
        for row in target_rows:
            lines.append(
                f"| `{row['batch_size']}` | `{row['mcmc_steps']}` | {row['status']} | "
...
run_job() {
    ...
    while ((attempt < 2)); do
        ...
        build_command cmd "${gpu_id}" "${run_name}" "${recipe}" "${target}" \
            "${seed}" "${config_path}" "${batch_size}" "${mcmc_steps}" \
            "${metric_family}"
...
declare -a JOBS=()
while IFS=$'\t' read -r run_id recipe target seed config_path batch_size mcmc_steps metric_family; do
    [[ "${run_id}" == "run_id" ]] && continue
    if [[ -f "${DONE_DIR}/${run_id}.done" && -f "${RESULT_MAP_DIR}/${run_id}.path" ]]; then
        continue
    fi
    JOBS+=(
        "${run_id}"$'\t'"${recipe}"$'\t'"${target}"$'\t'"${seed}"$'\t'"${config_path}"$'\t'"${batch_size}"$'\t'"${mcmc_steps}"$'\t'"${metric_family}"
    )
done <"${MANIFEST_PATH}"

echo "Pending runs: ${#JOBS[@]} / ${EXPECTED_RUNS}"
...
launch_available_jobs() {
    while ((${#FREE_GPUS[@]} > 0 && NEXT_JOB < ${#JOBS[@]})); do
        ...
        NEXT_JOB=$((NEXT_JOB + 1))
        local run_id recipe target seed config_path batch_size mcmc_steps metric_family
        IFS=$'\t' read -r run_id recipe target seed config_path batch_size mcmc_steps metric_family <<<"${job}"
