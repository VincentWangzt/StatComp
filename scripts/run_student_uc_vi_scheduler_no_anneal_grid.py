...
CAMPAIGN_SLUG = "student_uc_vi_step1000_gamma09_no_anneal"
CONFIG_HASH_VERSION = "student-uc-no-anneal-vi-step1000-gamma09-v1"
TARGET = "student_uc"
SEEDS = (42, 43, 44)
EPOCHS = 10000
...
def run_id_for(seed: int, variant_slug: str) -> str:
    return f"seed{seed}_10k_{variant_slug}_{TARGET}"
...
def build_variant_overrides(variant: dict[str, Any], user_overrides: list[str]) -> list[str]:
    overrides = [
...
def build_manifest_entries(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for seed in args.seeds:
        for variant in VARIANTS:
            config_path = base_config_path(str(variant["source_method"]))
            ...
            run_id = run_id_for(seed, str(variant["slug"]))
            ...
            entry = {
                ...
                "config_hash_basis": (
                    "resolved main config plus seed, Student-UC no-anneal override, "
                    "10k epoch override, VI StepLR step_size=1000/gamma=0.9, "
                    "variant overrides, and user extra overrides; target/vi/reverse "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Student-UC no-anneal VI StepLR gamma=0.9 grid with dynamic GPU scheduling."
    )
