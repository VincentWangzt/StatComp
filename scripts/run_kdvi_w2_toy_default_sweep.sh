...
# KDVI-W2 default toy-target sweep.
#
...
# final/best VI metrics across ten seeds for each target.

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
LOCAL_CAMPAIGN_SLUG="kdvi_w2_toy_default_sweep_10seed"
TRACKING_CAMPAIGN_SLUG="kdvi_dsivi_toy_default_sweep_10seed_new"
CAMPAIGN_ROOT="${REPO_ROOT}/campaigns/${LOCAL_CAMPAIGN_SLUG}"
...
usage() {
    cat <<'EOF'
Usage: bash scripts/run_kdvi_w2_toy_default_sweep.sh [options]

Options:
  --dry-run           Generate and validate the 70-run manifest only.
...
W&B tracking.campaign is shared with kdvi_dsivi_toy_default_sweep_10seed_new.
EOF
}
...
generate_manifest() {
    ...
    local target seed recipe run_id config_path

    printf 'run_id\trecipe_id\tmethod\ttarget\tseed\tconfig_path\n' >"${tmp_path}"

    for target in banana x_shaped multimodal 8_gaussians 8_gaussians_small student_uc Langevin_post; do
        recipe="KDVI-W2-${target}"
        config_path="configs/kdvi_w2_${target}.yaml"
        for seed in 0 1 7 42 43 44 45 46 47 48; do
            run_id="${recipe}-seed${seed}"
            printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
                "${run_id}" "${recipe}" "KDVI-W2" "${target}" "${seed}" \
...
errors = []
if len(rows) != 70:
    errors.append(f"expected 70 runs, found {len(rows)}")
if len(set(run_ids)) != 70:
    errors.append("run IDs are not unique")
if len(groups) != 7:
    errors.append(f"expected 7 target groups, found {len(groups)}")
bad_groups = {
...
print(
    "Manifest validated: "
    f"{len(rows)} runs, {len(groups)} target groups, "
    f"{len(methods)} method, {len(targets)} targets."
)
PY
}
...
build_command() {
    ...
    output_ref=(
        ...
        "seed=${seed}"
        "output.results_dir=results/${LOCAL_CAMPAIGN_SLUG}/${run_name}"
        "tracking.campaign=${TRACKING_CAMPAIGN_SLUG}"
...
if ((DRY_RUN)); then
    ...
    IFS=$'\t' read -r run_id recipe method target seed config_path <<<"${first_job}"
    cmd=()
    build_command cmd 0 "${run_id}" "${recipe}" "${seed}" "${config_path}"
    printf '  %q' "${cmd[@]}"
    printf '\n'
    exit 0
fi
...
rows = []
for recipe in sorted(recipe_meta, key=lambda key: recipe_meta[key]["target"]):
...
lines = [
    "# KDVI-W2 Toy Default Sweep Summary",
    "",
    f"Complete target groups: **{len(complete_rows)} / {len(rows)}**.",
...
lines.extend(["## Incomplete Target Groups", ""])
if incomplete:
...
run_job() {
    ...
    while ((attempt < 2)); do
        ...
        local -a cmd=()
        build_command cmd "${gpu_id}" "${run_name}" "${recipe}" "${seed}" "${config_path}"
...
declare -a JOBS=()
while IFS=$'\t' read -r run_id recipe method target seed config_path; do
    [[ "${run_id}" == "run_id" ]] && continue
    if [[ -f "${DONE_DIR}/${run_id}.done" && -f "${RESULT_MAP_DIR}/${run_id}.path" ]]; then
        continue
    fi
    JOBS+=("${run_id}"$'\t'"${recipe}"$'\t'"${seed}"$'\t'"${config_path}")
done <"${MANIFEST_PATH}"

echo "Pending runs: ${#JOBS[@]} / 70"
...
launch_available_jobs() {
    while ((${#FREE_GPUS[@]} > 0 && NEXT_JOB < ${#JOBS[@]})); do
        ...
        NEXT_JOB=$((NEXT_JOB + 1))
        local run_id recipe seed config_path
        IFS=$'\t' read -r run_id recipe seed config_path <<<"${job}"

        run_job "${gpu_id}" "${run_id}" "${recipe}" "${seed}" "${config_path}" &
        local pid=$!
