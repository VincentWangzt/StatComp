...
class CondMaskedAffineFlow(nf.flows.Flow):
    ...
    def forward(self, z, context):
        ...
        log_det = torch.sum((1 - self.b) * trafo_scale.log(),
    ...
    def inverse(self, z, context):
        ...
        #log_det = -torch.sum((1 - self.b) * scale, dim=list(range(1, self.b.dim())))
        log_det = -torch.sum(
            (1 - self.b) * trafo_scale.log(), dim=list(range(1, self.b.dim())))
        return z_, log_det
...
def generate_cond_real_nvp(K,
    ...
    for i in range(K):
        s = nf.nets.MLP([
            latent_size + context_size, 2 *
            (latent_size + context_size), latent_size
        ],
                        init_zeros=True,
                        leaky=0.0)
        t = nf.nets.MLP([
