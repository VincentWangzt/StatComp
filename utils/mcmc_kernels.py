...
@torch.no_grad()
def mala_transition_ivi(
    z_init: Tensor,
    score_fn: Callable[[Tensor], Tensor],
    logp_fn: Callable[[Tensor], Tensor],
    stepsz: float,
    anneal_coef: float,
    n_steps: int,
) -> MCMCTransitionOutput:
    """Bit-exact batched replica of ``ImVIDrift.mala`` from
    ``IVI-via-mcmc-distillation/run_ivi.py``.

    Reproduces the IVI notebook's MALA proposal and Metropolis-Hastings
    correction with the *identical floating-point operation order* so that,
    given the same RNG stream and inputs, KDVI's MCMC-refined samples are
    byte-identical to IVI's. The general :func:`mala_transition` is
    mathematically equivalent but uses a different (also valid) operation
    ordering; use this variant only on the KDVI ``mala`` parity path.

    The single-step update mirrors the notebook exactly::

        random_noise = randn_like(x)
        prop = x + stepsz*anneal_coef*score(x) + sqrt(2*stepsz)*random_noise
        logq_fwd  = sum(-0.5*log(2pi) - 0.5*log(2*stepsz) - 0.5*random_noise**2)
        bwd_mean  = prop + stepsz*anneal_coef*score(prop)
        logq_bwd  = sum(-0.5*log(2pi) - 0.5*log(2*stepsz)
                        - 0.5*(x - bwd_mean)**2 / (2*stepsz))
        log_ratio = anneal*logp(prop) + logq_bwd - anneal*logp(x) - logq_fwd
        accept    = log(rand_like(log_ratio)) < log_ratio

    Args:
        z_init: Starting positions ``[N, D]`` (detached).
        score_fn: RAW target score ``z -> [N, D]`` (NOT annealed; annealing is
            applied internally via ``anneal_coef``, matching IVI).
        logp_fn: RAW target log-density ``z -> [N]`` (NOT annealed).
        stepsz: IVI step size (``drift_stepsz / anneal_coef`` in the notebook).
            The proposal noise variance is ``2*stepsz``; the drift coefficient
            is ``stepsz*anneal_coef``.
        anneal_coef: Annealing factor (IVI's ``anneal_coef`` == KDVI's ``beta``).
        n_steps: Number of MALA transition steps (K).

    Returns:
        MCMCTransitionOutput with final positions, mean accept rate across batch
        and K steps, and mean displacement.
    """
    z = z_init.clone()
    total_accept_frac = 0.0
    log_2stepsz = math.log(2 * stepsz)
    sqrt_2stepsz = math.sqrt(2 * stepsz)

    for _ in range(n_steps):
        random_noise = torch.randn_like(z)
        prop_x = (
            z
            + stepsz * anneal_coef * score_fn(z)
            + sqrt_2stepsz * random_noise
        )
        ...
        backward_mean = prop_x + stepsz * anneal_coef * score_fn(prop_x)
        logq_prop_x_backward = torch.sum(
            -0.5 * math.log(2 * math.pi)
            - 0.5 * log_2stepsz
            - 0.5 * (z - backward_mean) ** 2 / (2 * stepsz),
            dim=-1,
        )

        curr_logp_x = anneal_coef * logp_fn(z)
        prop_logp_x = anneal_coef * logp_fn(prop_x)
        log_accept_ratio = (
        ...
        z = torch.where(accept.unsqueeze(-1), prop_x, z)
        total_accept_frac += accept.float().mean().item()

    accept_rate = total_accept_frac / max(1, n_steps)
    mean_disp = (z - z_init).norm(dim=-1).mean().item()
