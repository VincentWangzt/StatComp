...
class Toy_2D(object):
    ...
    def contour_plot(self,
        ...
        if quiver:
            ...
            scores = np.reshape(
                fnet(torch.Tensor(cpositions.T).to(
                    self.device)).detach().cpu().numpy(), cpositions.T.shape)
...
class Banana_shape(Toy_2D):
    ...
    def logp(self, X):
        ...
        sigmasqinv = torch.tensor([[1.0, -0.9], [-0.9, 1.0]]).to(
            self.device) / 0.19
        return -0.5 * 2 * np.log(2 * np.pi) - 0.5 * np.log(
            0.19) - 0.5 * torch.matmul(torch.matmul(Y[:, None, :], sigmasqinv),
                                       Y[:, :, None]).squeeze(-1)

    def score(self, X):
        Y = torch.matmul(
            torch.stack((X[:, 0], X[:, 0]**2 + X[:, 1] + 1), 1),
            torch.tensor([[1., -0.9], [-0.9, 1.]]).to(self.device))
        return -torch.stack(
            (Y[:, 0] + 2 * X[:, 0] * Y[:, 1], Y[:, 1]), 1) / 0.19
...
class X_shaped(Toy_2D):
    ...
    def logp(self, X):
        sigmasqinv_0 = torch.tensor([[2., -1.8], [-1.8, 2.]]).to(
            self.device) / 0.76
        sigmasqinv_1 = torch.tensor([[2., 1.8], [1.8, 2.]]).to(
            self.device) / 0.76
        return -0.5 * 2 * np.log(2 * np.pi) - 0.5 * np.log(
            0.76 * 4) + torch.logsumexp(torch.stack(
                (-1 / 2 *
                 torch.matmul(torch.matmul(X[:, None, :], sigmasqinv_0),
                              X[:, :, None]).squeeze(-1), -1 / 2 *
                 torch.matmul(torch.matmul(X[:, None, :], sigmasqinv_1),
                              X[:, :, None]).squeeze(-1)), 1),
                                        dim=1)
    ...
    def score(self, X):
        ...
        Y = F.softmax(torch.stack(
            (-1 / 2 * torch.matmul(torch.matmul(X[:, None, :], sigmasqinv_0),
                                   X[:, :, None]).squeeze(-1),
             -1 / 2 * torch.matmul(torch.matmul(X[:, None, :], sigmasqinv_1),
                                   X[:, :, None]).squeeze(-1)), 1),
                      dim=1)

        return -Y[:, 0] * torch.matmul(
            sigmasqinv_0, X[:, :, None]).squeeze(-1) - Y[:, 1] * torch.matmul(
                sigmasqinv_1, X[:, :, None]).squeeze(-1)
...
class Multimodal(Toy_2D):
    ...
    def logp(self, X):
        ...
        return -0.5 * 2 * np.log(2 * np.pi) - np.log(2.0) + torch.logsumexp(
            -torch.sum(
                (X.unsqueeze(1) - means.unsqueeze(0))**2, dim=-1) / 2. / 1**2,
            dim=1)

    def score(self, X):
        Y = F.softmax(torch.stack(
            (-1 / 2 * ((X[:, 0] + 2)**2 + X[:, 1]**2), -1 / 2 *
             ((X[:, 0] - 2)**2 + X[:, 1]**2)), 1),
                      dim=1)
        return -torch.stack(
            (Y[:, 0] * (X[:, 0] + 2) + Y[:, 1] * (X[:, 0] - 2), X[:, 1]), 1)
...
class StudentTFullDim(Toy_2D):
    ...
    def logp(self, X):
        ...
        B = X.shape[0]
        Z = torch.bmm(
            self.A_inv.unsqueeze(0).expand(B, -1, -1),
            X.unsqueeze(-1)).squeeze(-1)  # (B, D)
...
class LRwaveform(object):
    ...
    def logp(self, Z, batchdataset, batchlabel, scale_sto=1):
        ...
        logpy_xz = (batchlabel.reshape(-1, 1) * inner_prod +
                    F.logsigmoid(-inner_prod))
    ...
    def score(self, Z, batchdataset, batchlabel, scale_sto):
        ...
        inner_prod = torch.mm(batchdataset, W.t())
        score_W = -W * self.alpha + (YX - torch.sum(
...
class Bnn(object):
    ...
    def score(self, Z, batchdataset, batchlabel, scale_sto=1, max_param=50.0):
        ...
        nabla_wb = scale_sto * gamma_ * (
            (batchlabel - dnn_predict) * nabla_predict_wb).sum(1) - lambda_ * Z
    ...
    def rmse_llk(self,
        ...
        logpy_xz = -0.5 * (np.log(
            2 * np.pi) - log_gamma[:, None, :]) - 0.5 * gamma_[:, None, :] * (
                dnn_predict_true - batchlabel[None, :, :])**2
        test_llk = (torch.logsumexp(logpy_xz.squeeze(2), dim=0).mean() -
                    np.log(Z.shape[0]))
...
class Langevin_post(object):
    ...
    def __init__(self,
        ...
        self.upper_mask = torch.triu(
            torch.ones((self.dim, self.dim),
                       device=device)).contiguous().bool()
        self.upper1_mask = (
            1 - torch.triu(torch.ones(
                (self.dim, self.dim), device=device)).transpose(
                    0, 1)).contiguous().bool()
        ...
        for i in range(self.dim):
            u = u + self.beta * u * (1 - u**2) * self.dt + xs[i] * np.sqrt(
                self.dt)
            us.append(u)
    ...
    def score(self, us):
        ...
        score_prior_2 = -torch.concatenate(
            [(us_mean - us[:, 1:]) / (self.dt) *
             (1 - self.beta * self.dt - self.beta * self.dt * 2 *
              (us[:, :-1]**2 - 1) / (us[:, :-1]**2 + 1)**2),
    ...
    def trace_plot(self, u, figpath=None, figname=None, figtitle=""):
        ...
        u_mean = u.mean(0)
        low_CI_bound, high_CI_bound = st.t.interval(0.95,
                                                    len(u_mean),
                                                    loc=u_mean,
                                                    scale=np.std(u, 0))
