...
StatComp is a PyTorch research codebase implementing multiple Semi-Implicit Variational Inference (SIVI) algorithms. It provides a modular framework for comparing six SIVI variants (SIVI, UIVI, RSIVI, AISIVI, DSIVI, KSIVI) across various target distributions with comprehensive experiment management and evaluation.
...
**Local Windows note**: `requirements.txt` pins `triton==3.5.0`, which does not have Windows wheels. On this machine, the working local setup was created with `uv` and all other requirements installed into `.venv`; `triton` is intentionally omitted locally. The repo runs CPU-only on the local machine.
...
| `ksivi.py` | None | Kernel Stein objective without reverse-model training |
...
- **Python**: 3.14.3 (venv managed by `uv` 0.11.2, in `.venv\`)
- **PyTorch**: 2.9.0+cpu
- **OS**: Windows 10 x86_64
- **IMPORTANT**: Always run via `.\.venv\Scripts\python.exe` (or activate with `.\.venv\Scripts\activate`). The system Python environment does not have the project dependencies installed.
- **IMPORTANT**: `triton==3.5.0` is Linux-only, so it is not installed in the local Windows `.venv`.
